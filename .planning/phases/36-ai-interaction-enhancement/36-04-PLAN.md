...
must_haves:
  ...
  key_links:
    ...
    - from: "ConversationScreen"
      to: "MessageBubble"
      via: "passing projectId prop"
      pattern: "MessageBubble.*projectId"
---
...
<task type="auto">
  ...
  <files>frontend/lib/screens/conversation/widgets/message_bubble.dart, frontend/lib/screens/conversation/conversation_screen.dart</files>
  <action>
1. Update `frontend/lib/screens/conversation/widgets/message_bubble.dart` to show source chips for assistant messages:
...
2. Update `frontend/lib/screens/conversation/conversation_screen.dart` to pass projectId to MessageBubble:

In the _buildMessageList method where MessageBubble is constructed:

```dart
// When constructing MessageBubble, pass projectId:
MessageBubble(
  message: message,
  projectId: widget.projectId,  // Pass project ID for document navigation
)
```

NOTE: The conversation_screen.dart likely already has access to projectId via widget.projectId or similar. Verify the exact property name and pass it to MessageBubble.
  </action>
  <verify>
```bash
cd frontend && flutter analyze lib/screens/conversation/widgets/message_bubble.dart lib/screens/conversation/conversation_screen.dart
```
  </verify>
  <done>
- MessageBubble shows SourceChips for assistant messages with documentsUsed
- MessageBubble accepts projectId for document navigation
- ConversationScreen passes projectId to MessageBubble
