...
Per PITFALL-05: Track actual documents searched at backend level, not LLM-reported documents.
  ...
  <verify>
```bash
cd backend && grep -n "_documents_used_context" app/services/agent_service.py
```
Verify output shows context variable definition and usage in search_documents_tool.
  </verify>
  <done>
