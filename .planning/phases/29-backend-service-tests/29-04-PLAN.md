...
Target 80% coverage to match phase success criteria. The ai_service module has some edge cases in heartbeat and exception handling that may remain uncovered, but core paths should achieve the threshold.
  </action>
  <verify>
Coverage report shows 80%+ line coverage for ai_service.py
Tests run without requiring API keys or network access
  </verify>
  <done>ai_service achieves 80%+ test coverage with no real API calls</done>
</task>
...
6. ai_service achieves 80%+ line coverage
