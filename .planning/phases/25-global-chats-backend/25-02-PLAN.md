...
must_haves:
  ...
  key_links:
    ...
    - from: "ResponsiveScaffold"
      to: "ChatsScreen"
      via: "Navigation rail/drawer item at index 1"
      pattern: "Chats"
    - from: "main.dart routes"
      to: "ConversationScreen"
      via: "/chats/:threadId route with null projectId"
...
<task type="auto">
  <name>Task 3: Make ConversationScreen.projectId optional</name>
  <files>
    frontend/lib/screens/conversation/conversation_screen.dart
  </files>
  <action>
...
1. Change projectId to be nullable:
   ```dart
   class ConversationScreen extends StatefulWidget {
     ...
     const ConversationScreen({
       super.key,
       this.projectId,  // Changed from required
       required this.threadId,
     });
   ```

2. Update any code in the file that references widget.projectId to handle null:
   - Any breadcrumb or project-related UI elements should check for null
   - Back navigation should handle both project-based and project-less cases
   - If there's a "back to project" button, it should be conditional on hasProject
...
4. The loadThread() call in initState should work unchanged since it only uses threadId.

Note: This is a minimal change to enable routing. The conversation functionality itself does not depend on projectId - it operates on the thread.
  </action>
  <verify>
...
```
No errors. ConversationScreen accepts optional projectId.
  </verify>
  <done>
ConversationScreen.projectId is now optional (String?). Project-less threads can be displayed without projectId.
  </done>
</task>

<task type="auto">
  <name>Task 4: Add navigation and routes for Chats</name>
  <files>
    frontend/lib/widgets/responsive_scaffold.dart
    frontend/lib/main.dart
  </files>
  <action>
Update frontend/lib/widgets/responsive_scaffold.dart:

1. Add Chats navigation item to _destinations list (after Home, before Projects):
   ```dart
   static const List<_NavigationDestination> _destinations = [
     ...
     // NEW: Chats destination at index 1
     _NavigationDestination(
...
1. Import ChatsProvider and ChatsScreen:
   ```dart
   import 'providers/chats_provider.dart';
   import 'screens/chats_screen.dart';
   import 'services/api_service.dart';  // May already exist
   ```

2. Register ChatsProvider in MultiProvider (instantiate ApiService directly, NOT via context.read):
   ```dart
   // In MultiProvider providers list, add:
   ChangeNotifierProvider(
     create: (_) => ChatsProvider(ApiService()),
   ),
   ```

3. Update _getSelectedIndex to include Chats at index 1:
   ```dart
   int _getSelectedIndex(String path) {
     if (path.startsWith('/home')) return 0;
     if (path.startsWith('/chats')) return 1;  // NEW
     if (path.startsWith('/projects')) return 2;  // Changed from 1
     if (path.startsWith('/settings')) return 3;  // Changed from 2
     return 0; // Default to home
   }
   ```

4. Add Chats branch to StatefulShellRoute.indexedStack branches (at index 1):
   ```dart
   branches: [
     // Branch 0: Home
     StatefulShellBranch(
     ...
     // Branch 2: Projects (was Branch 1)
     StatefulShellBranch(
       routes: [
         GoRoute(
           path: '/projects',
           ...
           routes: [
             // ... existing project routes unchanged
           ],
         ),
       ],
     ),
     // Branch 3: Settings (was Branch 2)
     StatefulShellBranch(
       routes: [
         GoRoute(
           path: '/settings',
...
IMPORTANT: ApiService is NOT a provider - it's instantiated directly. Do NOT use context.read<ApiService>().
  </action>
  <verify>
Run the app and verify:
1. Chats appears in navigation rail/drawer after Home
2. Clicking Chats shows ChatsScreen
3. _getSelectedIndex returns correct index for /chats paths
4. /chats/:threadId route works with null projectId
```bash
cd frontend && flutter run -d chrome
```
  </verify>
  <done>
Navigation updated with Chats at index 1. Routes added for /chats and /chats/:threadId. Index calculation updated for new destination ordering.
  </done>
</task>

<task type="auto">
  <name>Task 5: Add New Chat button to Home screen</name>
  <files>
    frontend/lib/screens/home_screen.dart
  </files>
  <action>
...
2. Add New Chat button to _buildActionButtons method (before "Start Conversation"):
   ```dart
...
Note: This replaces the previous "Start Conversation" (which just went to projects) with "New Chat" that creates a project-less thread immediately.
  </action>
  <verify>
Run app and verify Home page has New Chat button that creates thread and navigates:
```bash
cd frontend && flutter run -d chrome
```
Click New Chat on home page - should create thread and navigate to /chats/{threadId}.
  </verify>
  <done>
Home page has New Chat button (CHATS-06) that creates project-less thread and navigates to conversation view.
  </done>
</task>
...
<verification>
1. Chats navigation:
   - Chats section visible in navigation rail/drawer
   - Position: After Home (index 1), before Projects (index 2)
...
4. Navigation (CHATS-07):
   ...
   - Conversation view loads for project-less thread (projectId: null)
   - Can send messages in project-less thread

5. Project-based threads still work:
   - Existing thread routes unchanged (/projects/:id/threads/:threadId)
   - Creating thread within project still works
   - Navigation from Chats list to project threads works
</verification>
