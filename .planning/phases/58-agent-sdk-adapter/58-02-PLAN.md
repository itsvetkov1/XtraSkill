...
must_haves:
  truths:
    ...
    - "Unit tests verify HTTP-based MCP transport is configured (not ContextVars)"
    - "Unit tests verify db session register/unregister lifecycle"
...
<task type="auto">
  ...
  <action>
    ...
    **TestClaudeAgentAdapterStreamChat:**
    ...
    10. `test_http_mcp_transport_configured` — Mock query() and verify that ClaudeAgentOptions
        passed to query() contains HTTP MCP server config with `"type": "url"` and headers
        including X-Project-ID, X-Thread-ID, X-DB-Session-ID, X-Max-Results. This verifies the
        locked decision: HTTP-based MCP transport, not ContextVars.

    11. `test_db_session_registered_and_unregistered` — Mock query() and verify that
        `register_db_session` is called before query() and `unregister_db_session` is called
        after streaming completes (in finally block). Test both success and error paths.

    12. `test_documents_used_from_tool_result_marker` — Mock query() to yield a ToolResultBlock
        with DOCUMENTS_USED marker. Verify the complete chunk's metadata contains the parsed
        documents_used data (not from ContextVar, but from marker parsing).

    **TestClaudeAgentAdapterInit (keep existing + extend):**

    13. Keep existing init tests (api_key, default_model, custom_model, provider)
    14. Add: `test_mcp_server_url_set` — Verify mcp_server_url attribute is set (not None) after init,
        confirming HTTP MCP server URL is obtained via get_mcp_http_server_url()
    15. Add: `test_is_agent_provider` — Verify `is_agent_provider` is True
    16. Add: `test_set_context` — Verify set_context stores db/project_id/thread_id
    ...
    **Important patterns:**
    ...
    - Mock `get_mcp_http_server_url` to return a fake URL (avoid starting real HTTP server)
    - Mock `register_db_session` and `unregister_db_session` to verify session lifecycle
    - Verify HTTP headers are passed in ClaudeAgentOptions (not ContextVars)
  ...
  <done>
    ...
    - HTTP MCP transport configured with correct headers (not ContextVars)
    - DB session register/unregister lifecycle
    - Documents used parsed from tool result markers
    - MCP HTTP server URL set on init
...
<success_criteria>
1. ClaudeAgentAdapter has 16+ tests covering all SDK event translation paths
...
6. Tests verify HTTP-based MCP transport with context headers (not ContextVars) per locked decision
</success_criteria>
