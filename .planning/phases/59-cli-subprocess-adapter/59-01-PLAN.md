...
must_haves:
  truths:
    ...
    - "Adapter fails clearly when CLI is not installed (RuntimeError with install instructions)"
...
4. Build CLI command: `[self.cli_path, "-p", "--output-format", "stream-json", "--verbose", "--model", self.model, prompt_text]`
   - Do NOT pass `--system-prompt` flag. Instead, prepend the system prompt to the prompt text with a clear delimiter: `[SYSTEM]: {system_prompt}\n\n[USER]: {prompt_text}`
   - Do NOT pass `--include-partial-messages` flag — excluded from POC scope (document as known limitation in docstring: "Partial message streaming not supported; only complete events are emitted")
...
**MCP tool integration (POC approach):**
For POC, tools are communicated via the system prompt prepended to the prompt text. The system prompt already describes the `search_documents` and `save_artifact` tools with their parameters and expected JSON output format. The CLI's own agent loop reads these instructions and calls the tools autonomously using its built-in tool handling. No `.mcp.json` file or MCP server configuration is needed for POC — the CLI acts as a self-contained agent that interprets tool descriptions from the prompt. Tool use events from the CLI are translated to `StreamChunk(chunk_type="tool_use")` by `_translate_event()`, satisfying the CLI-04 requirement.

**What NOT to implement (deferred to production):**
- MCP tool configuration via temporary `.mcp.json` file (production optimization — would give CLI direct MCP server access)
